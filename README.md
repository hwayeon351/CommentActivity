...
## 동작 과정
### 1. 댓글 로드
* journal_id를 담은 요청을 서버에 보내 comments 데이터를 응답 받는다. 각각의 댓글 데이터를 불러오기 위해 아래 과정을 comments에 담긴 comment_id 개수만큼 반복한다.
 ** comment_id를 담은 요청을 서버에 보내고 user_id, comment_date, comment 데이터를  응답 받는다.
 ** user_id를 담은 요청을 서버에 보내고 nickname을 응답 받는다. nickname,  comment_date, comment를 담은 하나의 댓글 항목을 전체 댓글 리스트에 추가한다. 마지막 댓글 항목이라면 RecyclerView를 갱신한다.
